...
#include "Flash/DueFlashStorage.h"
...
void Platform::Init()
{
	...
	if (nvData.magic != FlashData::magicValue)
	{
		...
		{
			nvData.ipAddress[i] = ip_address[i];
			nvData.netMask[i] = net_mask[i];
			nvData.gateWay[i] = gate_way[i];
		}
		for(int8_t i = 0; i < 6; i++)
		{
			nvData.macAddress[i] = mac_address[i];
		}
	...
	{
		stepPins[i] = step_pins[i];
		directionPins[i] = direction_pins[i];
		enablePins[i] = enable_pins[i];
		disableDrives[i] = disable_drives[i];
		lowStopPins[i] = low_stop_pins[i];
		highStopPins[i] = high_stop_pins[i];
		maxFeedrates[i] = max_feedrates[i];
		accelerations[i] = accelerations_[i];
		driveStepsPerUnit[i] = drive_steps_per_unit[i];
		instantDvs[i] = instant_dvs[i];
		potWipes[i] = pot_wipes[i];
	}
	...
	{
		axisMaxima[i] = axis_maxima[i];
		axisMinima[i] = axis_minima[i];
		homeFeedrates[i] = home_feedrates[i];
		headOffsets[i] = head_offsets[i];
	}
	...
	{
		tempSensePins[i] = temp_sense_pins[i];
		heatOnPins[i] = heat_on_pins[i];
		standbyTemperatures[i] = standby_temperatures[i];
		activeTemperatures[i] = active_temperatures[i];
	}
