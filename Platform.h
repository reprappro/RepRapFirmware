...
#define DRIVE_ENABLE false      						// What to send to enable...
#define DRIVE_DISABLE true     						// ...and disable a drive
...
#define DIRECTIONS {FORWARDS, FORWARDS, FORWARDS, FORWARDS} // What each axis needs to make it go forwards - defaults
...
inline void Platform::Disable(byte drive)
{
	...
	if(drive == Z_AXIS || drive==E0_DRIVE || drive==E2_DRIVE) //ENABLE_PINS {29, 27, X1, X0, 37, X8, 50, 47}
		digitalWriteNonDue(enablePins[drive], DRIVE_DISABLE);
	else
		digitalWrite(enablePins[drive], DRIVE_DISABLE);
...
inline void Platform::Step(byte drive)
{
	...
	{
		if(drive == Z_AXIS || drive==E0_DRIVE || drive==E2_DRIVE) //ENABLE_PINS {29, 27, X1, X0, 37, X8, 50, 47}
			digitalWriteNonDue(enablePins[drive], DRIVE_ENABLE);
		else
			digitalWrite(enablePins[drive], DRIVE_ENABLE);
