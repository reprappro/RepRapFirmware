...
const size_t DDA_RING_LENGTH = 20;
...
class Move
{
	public:
		...
		const float IdleTimeout() const;												// Returns the idle timeout in seconds
		void SetIdleTimeout(const float timeout);										// Set the idle timeout in seconds
		...
		FilePosition PausePrint(float positions[DRIVES+1], unsigned int &skippedMoves);	// Pause the print as soon as we can
		...
		float GetExtrusionFactor(uint8_t extruder) const;								// Factor by which we change the extrusion amount since the last move
	...
	private:
		...
		volatile float rawExtruderTotals[DRIVES - AXES];								// The raw and unmodified extruder distances
...
inline const float Move::IdleTimeout() const
{
	return idleTimeout;
}

inline void Move::SetIdleTimeout(const float timeout)
{
	idleTimeout = timeout;
}
