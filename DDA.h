...
class DDA
{
	public:
		...
		bool FetchEndPosition(volatile int32_t ep[DRIVES], volatile float endCoords[AXES], volatile float rawExtrDists[DRIVES - AXES]);
	...
	private:
		...
		float rawExtruderDistances[DRIVES - AXES];								// Store the raw extruder distances as well for better print time estimation
